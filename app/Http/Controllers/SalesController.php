...
class SalesController extends BaseController
{
    ...
    public function index(request $request)
    {
        ...
        // Pure sale time
        $to_time = date("H:i:s", strtotime(str_replace('/', '-', $request->to)));
        $from_time = date("H:i:s", strtotime(str_replace('/', '-', $request->from)));
        // Pure sale date
        ...
        $Sales = Sale::with('facture', 'client', 'warehouse','user')
            ...
            ->whereBetween('sales.time', array($from_time, $to_time))
        ...
        foreach ($Sales as $Sale) {
            ...
            $item['user_name'] = $Sale['user']['firstname'] . " " . $Sale['user']['lastname'];
        ...
        $users = User::where('deleted_at', '=', null)->get(['id', 'firstname', 'lastname']);
    ...
    public function store(Request $request)
    {
        ...
        \DB::transaction(function () use ($request) {
            ...
            $time = date("H:i:s", strtotime(str_replace('/', '-', $request->date)));
            ...
            if (SaleReturn::where('sale_id', $id)->where('deleted_at', '=', null)->exists()) {
                ...
                foreach ($new_sale_details as $prd => $prod_detail) {
                    ...
                    if($prod_detail['sale_unit_id'] !== null || $get_type_product == 'is_service'){
                        ...
                        $orderDetails['time']         = $request['time'];
    ...
    public function Sale_PDF(Request $request, $id)
    {
        ...
        $item['date'] = $sale_data['date'] . ' ' . $sale_data['time'];
    ...
    public function create(Request $request)
    {
      ...
      public function edit(Request $request, $id)
      {
        if (SaleReturn::where('sale_id', $id)->where('deleted_at', '=', null)->exists()) {
          ...
          // Parse the date and create a Carbon instance
            $date = Carbon::parse($Sale_data->date);

            // Extract hours and minutes from the time string
            $timeParts = explode(':', $Sale_data->time);
            $hours = $timeParts[0];
            $minutes = $timeParts[1];

            // Set the time portion using the extracted hours and minutes
            $date->setTime($hours, $minutes);

            // Now you have the combined datetime in the Carbon instance
            $combinedDateTime = $date;

            // Format the combined datetime as needed
            $combinedDateTimeFormatted = $combinedDateTime->format('d/m/Y H:i');

          $sale['date'] = $combinedDateTimeFormatted;
