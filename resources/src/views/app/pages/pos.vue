  ...
  <div class="pos_page">
    <div class="container-fluid p-0 app-admin-wrap layout-sidebar-large clearfix" id="pos">
      ...
      <b-row v-if="!isLoading">
                      ...
                      <validation-provider name="Customer" :rules="{ required: true}">
                        <b-input-group slot-scope="{ valid, errors }" class="input-customer">
                          ...
                          <b-input-group-append>
                            <b-button v-if="currentUserPermissions && currentUserPermissions.includes('edit_product_sale')" variant="primary" @click="New_Client()">
                              <span>
        ...
        <button @click.prevent="logoutUser" class="position-fixed btn text-white btn-danger pos-logout">
          <i class="i-Lock-User mr-1 h4"></i>
          <!-- <span class="h4">{{currentUser.username}}</span> <br/> -->
          <span class="h4 text-white">{{currentUser.username}}</span>
        </button>
        <!-- Card right Of Products - listing categories -->
        <b-col md="7">
          <b-card class="list-grid">
            <b-row>
              <b-col md="6">
                <button v-b-toggle.sidebar-category class="btn btn-info mt-1 btn-block">
                  <i class="i-Two-Windows"></i>
                  {{$t('ListofCategory')}}
                </button>
              </b-col>
              <b-col md="6">
                <button v-b-toggle.sidebar-brand class="btn btn-info mt-1 btn-block">
                  <i class="i-Library"></i>
                  {{$t('ListofBrand')}}
                </button>
              </b-col>
              ...
              <div class="col-md-12 d-flex flex-row flex-wrap bd-highlight list-item mt-2">
                <div
                  ...
                  class="card o-hidden bd-highlight mx-1"
                >
                  ...
                  <div class="flex-grow-1 d-bock">
                    ...
                    >
                      <div class="w-40 w-sm-200 item-title font-base font-semibold">{{product.name}}</div>
                      <span
                        class="font-bold font-lg text-black item-price"
            ...
            <b-row>
              <div class="col-md-12 d-flex flex-row flex-wrap bd-highlight list-item mt-2">
                <div
                  ...
                  class="card o-hidden bd-highlight mx-1"
                >
                  <div class="list-thumb d-flex list-card-img">
                      <img alt :src="'/images/no-image.png'">
                  </div>
                  <div class="flex-grow-1 d-bock">
                    <div
                      class="card-body align-self-center d-flex flex-column justify-content-between align-items-lg-center"
                    >
                      <div class="item-title pos-card-title">{{$t('All_Category')}}</div>
                    </div>
                  </div>
                </div>
                <div
                ...
                >
                  <div class="list-card-img">
                    <img alt :src="'/images/categories/' + category.image">
                  </div>
                  <div class="flex-grow-1 d-bock">
                    <div
                      class="card-body align-self-center d-flex flex-column justify-content-between align-items-lg-center"
                    >
                      <div class="item-title pos-card-title">{{category.name}}</div>
                    </div>
                  </div>
                </div>
              </div>
        ...
        <b-modal hide-footer size="sm" scrollable id="Show_invoice" :title="$t('Invoice_POS')">
          ...
          <div id="invoice-POS">
            <div style="max-width:400px;margin:0px auto; margin-bottom:50px;">
              <div class="info">
                <p>
                  <h3>Bon de commande</h3>
                  <span>{{$t('date')}} : {{invoice_pos.sale.date}} {{invoice_pos.sale.time}} <br></span>
                  <span v-show="pos_settings.show_Warehouse">{{$t('reference')}} : {{invoice_pos.sale.Ref}} <br></span>
                  <span v-show="pos_settings.show_Warehouse">{{$t('serveur')}} : {{currentUser.username}} <br></span>
                </p>
              </div>

              <table class="table_data" style="margin-bottom: 10px;">
                <tbody>
                  <tr v-for="detail_invoice in invoice_pos.details">
                    <td colspan="3">
                      <span>{{formatNumber(detail_invoice.quantity,0)}}</span> {{detail_invoice.name}}
                    </td>
                  </tr>
            ...
            <div style="max-width:400px;margin:0px auto; margin-bottom:30px;">
              <div class="info">
              ...
              <div id="legalcopy" class="ml-2">
                ...
                <div id="bar" v-show="pos_settings.show_barcode" style="margin-bottom: 30px;">
                  <barcode
                    class="barcode"
...
export default {
  ...
  methods: {
    ...
    Submit_Payment() {
      // Start the progress bar.
      console.log("submit payement") // .boukali
      NProgress.start();
      NProgress.set(0.1); // .boukali submit payement
    ...
    //-------------------------------- Invoice POS ------------------------------\\
    Invoice_POS(id) {  // .boukali invoice and print
      // Start the progress bar.
      NProgress.start();
      NProgress.set(0.1);
      axios
        ...
        .then(response => {
          ...
          setTimeout(() => {
            ...
            NProgress.done();
            console.log("show Show_invoice");
            this.$bvModal.show("Show_invoice"); // .boukali prevent showing invoice before printing
          }, 500);

          if(response.data.pos_settings.is_printable){
            setTimeout(() => {
              console.log("hide show_invoice");
              this.$bvModal.hide("Show_invoice");
              this.print_pos()
            }, 1000);
          }
        })
    ...
    //----------------------------------Process Payment ------------------------------\\
    async processPayment() { // .boukali payement with stirpe (credit card)
    ...
    //----------------------------------Create POS ------------------------------\\
    CreatePOS() { // .boukali post paiement cach
      NProgress.start();
      NProgress.set(0.1);
    ...
    GetElementsPos() {
      axios
        ...
        .then(response => {
          ...
          this.paginate_Brands(response.data.brands.length, 0);
          this.paginate_Category(response.data.categories.length, 0);
  ...
  //-------------------- Created Function - .boukali pay_now -----\\
  created() {
    this.GetElementsPos();
    Fire.$on("pay_now", () => {
      setTimeout(() => {
